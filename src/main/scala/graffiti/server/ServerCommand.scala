 ...
 */
class ServerCommand(implicit cxt: Context) extends CliCommand("server", Some("Run HTTP server")) with Logging {
  ...
  override def run()(implicit exc: ExecutionContext): Unit =  {
    ...
    IO(Http) ? Http.Bind(system.actorOf(ServiceActor.props(cxt.service)),
      interface = interface,
      port = port)
  }
}
